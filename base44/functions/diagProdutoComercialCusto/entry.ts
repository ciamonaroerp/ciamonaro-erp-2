...
Deno.serve(async (req) => {
  try {
    ...
    const { data: precosSync, error: errorPrecos } = await base44.asServiceRole.client
    ...
    const { data: artigos, error: errorArtigos } = await base44.asServiceRole.client
      .from('produto_comercial_artigo')
      .select('id, codigo_unico, artigo_nome, status_rendimento, variavel_index, produto_id')
      .eq('produto_id', produto_id);

    const { data: produto, error: errorProduto } = await base44.asServiceRole.client
